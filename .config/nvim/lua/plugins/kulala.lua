-- Kulala REST client for Neovim. Global actions live under <leader>R (Space then R).
-- Depends on lazy.nvim for loading; which-key group is declared in lua/plugins/which-key.lua.
return {
  "mistweaverco/kulala.nvim",
  -- Load before session save/restore so VimLeavePre and SessionLoadPost hooks are registered.
  event = { "SessionLoadPost", "VimLeavePre" },
  -- Desc-only keys lazy-load the plugin; Kulala registers the real mappings in setup.
  keys = {
  ...
  opts = {
    ...
    -- Default keymaps use global_keymaps_prefix (<leader>R): Space, then R, then the action key.
    -- See :help kulala.default-keymaps or lua/kulala/config/keymaps.lua in the plugin.
    ---@type boolean|table
    global_keymaps = true,
    --[[
        {
